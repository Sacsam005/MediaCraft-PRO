...
function addDownloadAllButton() {
  if (!downloadAllButton) {
    ...
    downloadAllButton.setAttribute(
      "class",
      "py-1 px-2 mx-1 bg-success text-light rounded-0 border-0"
    );
    ...
    //   const downloadPromises = Array.from(videoTags).map((videoTag, index) => {
    ...
    for (const file of uploadedFiles) {
      ...
      reader.onload = async (e) => {
        ...
        fileList.innerHTML += `<li>
           ...
           <button class="p-1 border border-success download_btn" data-filename="${fileName}">Download</button>
           </li>`;
      };
